...
def student():
    ...
    Label(slave, text='Гусакова Вікторія\n'
                      'група ІВ-82\n'
                      'варіант 30\n'
                      '((7+82%60)%30)+1 = {}'.format(((7 + 82 % 60) % 30) + 1),
...
def window2():
    ...
    def show():
        ...
        Label(lf, text='\n'
              ...
              .format(
                    ...
                    f4=(U - C) & (A - C),
                    f5=(U - C) & (A - C) & (B - C),
                    rez=zad_alg.func_1_full(A, B, C, U)),
    ...
    Label(slave, text='Заданий вираз:\n'
                      'D = ¬C & (A - C) & (B - C) & (¬C | B) = {}\n'.format(zad_alg.func_1_full(A, B, C, U)),
          font='Arial 14 bold').grid(column=0, row=0, sticky=W, columnspan=2)

    Button(slave, text="Pозв'язок", font="Arial 12",
           command=show).grid(column=0, row=3)

    but = Button(slave, text='Зберегти ', font='Arial 12')

    but.grid(column=1, row=3)

    but.bind("<Button-1>", save_to_file_2(zad_alg.func_1_full(A, B, C, U)))
...
def window3():
    ...
    def show():
        ...
        Label(lf, text='\n'
              ...
              .format(
                    f1=(A & B),
                    rez1=zad_alg.func_1_small(A, B, C, U)),
              font='Arial 14', justify=LEFT).grid(column=0, row=5, sticky=W, columnspan=4)
    ...
    Label(slave, text='Заданий вираз:\n'
                      'D = (A - (C | B)) = {}\n'.format(zad_alg.func_1_small(A, B, C, U)),
          font='Arial 14 bold').grid(column=0, row=0, sticky=W, columnspan=2)
    ...
    but.grid(column=1, row=3)
    but.bind("<Button-1>", save_to_file_3(zad_alg.func_1_small(A, B, C, U)))
    but.bind("<Button-1>", but_disable)
...
def window5():
    ...
    Label(lf1, text='D(заданий алгоритм) = ¬C & (A - C) & (B - C) & (¬C | B) =\n\t\t= {}'.format(d1),
